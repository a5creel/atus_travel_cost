...
Indirect utility (extension with weather included):
...
# read in who must be dropped 
myDropped <-  vroom("clean_data/2.rec_and_trvl_indicator_ALL.csv") %>% 
  select(-date)

# merge in those who didn't both travel and participate in outdoor rec 
myRUM_idx_thin <- left_join(myRUM_df, myDropped, by = "caseid") %>% 
  mutate(choice = if_else(activity == "outdoor_away", rec_away_travel_and_choice, choice)) %>% 
  
  # dropping the new no leisure 
  group_by(caseid) %>% 
  mutate(new_no_leisure = sum(choice)) %>% 
  ungroup() %>% 
  filter(new_no_leisure !=0) %>% 
...
# Did rec AND traveled for it 
reg3 <- mlogit(choice ~ travel_time_total_state, 
               myRUM_idx_thin, 
               reflevel = "indoor_home") 
...
annual_CS <-  ggplot(myTotalWelfare_outdoor, aes(x = year, y = mean_cs_per_year_dollars/10^9)) +
  geom_point(size = 3.5) +
  geom_smooth(method = "lm", se = F, size = 2) + 
  labs(x = "Year", y = "Consumer Surplus (Billion dollars)") +
  ggtitle("Annual Change in Consumer Surplus", subtitle = "Counter-factual: Travel time increases to 10K minutes") +
  theme_bw() + 
  theme(text = element_text(size=30))
annual_CS

ggsave(annual_CS, width = 13, height = 8, filename = "figures/annual_CS.jpeg")
